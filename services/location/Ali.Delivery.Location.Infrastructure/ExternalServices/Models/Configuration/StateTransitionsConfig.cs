...
public record TransitionConfig(BotState NextState, NotificationType? Notification = null);
...
public class StateTransitionsConfig : Dictionary<BotState, Dictionary<string, TransitionConfig>>
{
    ...
    public Dictionary<string, TransitionConfig> GetAvailableTransitions(BotState currentState) =>
        TryGetValue(currentState, out var transitions) ? transitions : new Dictionary<string, TransitionConfig>();
    ...
    public TransitionConfig? GetTransition(BotState currentState, string transitionKey) =>
        TryGetValue(currentState, out var stateTransitions) && stateTransitions.TryGetValue(transitionKey, out var transition) ? transition : null;
}
